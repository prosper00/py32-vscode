...
ifeq ($(FLASH_PROGRM),jlink)
	...
	$(PYOCD_EXE) erase -t $(PYOCD_DEVICE) --chip 
#--config $(TOP)/Misc/pyocd.yaml
	$(PYOCD_EXE) load $(BDIR)/$(PROJECT).bin -t $(PYOCD_DEVICE) 
# --config $(TOP)/Misc/pyocd.yaml
else ifeq ($(FLASH_PROGRM),probers)
	probe-rs download --format bin ./Build/example-blink.bin --chip-erase --base-address 0x8000000 --verify --chip py32f030x8
else
